    ...
    <ul v-if="value.length > 0"
      ...
      <li v-for="rowItem in value"
        ...
        <div v-for="col in columns"
          ...
          <router-link
            v-if="col.linkTo"
            :id="`list-item_${rowItem.id}-${col.column}`"
            :to="col.linkTo(rowItem.id)">
            {{ col.rowValue }}
          </router-link>
          <span
            v-else
            :id="`list-item_${rowItem.id}-${col.column}`">
            {{ displayRowItemValue(rowItem[col.column]) }}
          </span>
        </div>
...
.table {
  ...
  &-list {
    ...
    &-item {
      ...
      @include breakpoint(desktop) {
        padding: 1.4rem 3.25vw 1.4rem 4vw;
      }
